...
unsigned char textscreen[TEXT_SIZE_X][TEXT_SIZE_Y];

unsigned char lineptr[TEXT_SIZE_Y]; // Zeiger auf Zeile (wird für das Scrollen benötigt)
unsigned char linecnt[TEXT_SIZE_Y]; // Zähler für Anzahl Zeichen in Zeilen
...
// flags
unsigned char gdp_char_direction;		// direction of textoutput (8=vertical, 0=horizontal)
unsigned char gdp_char_tilted;			// type of character output(4=tilted, 0=normal)
unsigned char gdp_char_scalex;			// character scaling 0..16 (0=16)
unsigned char gdp_char_scaley;
unsigned char gdp_page;				// view- and write-page
unsigned char gdp_cursor_x;			// position of text cursor (origin is top-left
unsigned char gdp_cursor_y;
unsigned int  gdp_xpos;				// position of graphics cursor (origin is bottom-left !!)
unsigned int  gdp_ypos;
unsigned char gdp_fgcolor;			// foreground color
unsigned char gdp_bkcolor;			// backgroud color
unsigned char gdp_scrollreg;			// copy of scroll register
unsigned char gdp_mode;				// GDP mode text 80x25, 40x25 or graphics 512x256
unsigned char gdp_transparent;			// 0=transparent mode off, 1=transparent mode on (GDP-FPGA)
unsigned char gdp_max_cx;			// current maximum text cursor position
unsigned char gdp_max_cy;
unsigned int gdp_max_gx;			// current maximum graphic cursor position
unsigned int gdp_max_gy;
