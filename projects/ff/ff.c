	...
	if (*path) {	/* If the pointer is not a null */
		...
		if (*tt == ':') {	/* If a ':' is exist in the path name */
			...
			if (i < 10 && tp == tt) {	/* Is there a numeric drive id? */
				...
				ff_dbg("   numeric drive number found: %d...\n",vol);	
				...
				ff_dbg("   no numeric drive number found...\n");
				...
				if (i < _VOLUMES) {	/* If a drive id is found, get the value and strip it */
					ff_dbg("    found a string drive id...\n");
					vol = (int)i;
					*path = tt;
				}
#endif
		...
		ff_dbg(" no ':' found, last char is %c\n",*tt);

#if _FS_RPATH && _VOLUMES >= 2
		ff_dbg("   set vol to CurrVol=%d...\n",CurrVol);
...
)
{
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	ff_dbg(" attemting to mount the volume ...  \n");
	...
	ff_dbg(" FAT volume found ...\n");
	
	
	if (LD_WORD(fs->win+BPB_BytsPerSec) != SS(fs)){		/* (BPB_BytsPerSec must be equal to the physical sector size) */
		ff_dbg(" fs->win+BPB_BytsPerSec = %d  -- SS(fs) = %d",LD_WORD(fs->win+BPB_BytsPerSec),SS(fs));
...
//		ff_dbg(" loop through FAT entries (%d)",n_fat);
